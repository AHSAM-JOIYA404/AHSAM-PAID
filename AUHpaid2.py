# Decompiled By AHSAM UL HAQ JOIYA 
# Github : https://github.cogiMR-AUH404
## LO GANTENG :V 
## JEECK COOL BOY
...
def approval():
  os.system('git pull')
  time.sleep(1)
  uuid = str(os.geteuid())+"V3"+str(os.geteuid())
  id = "AHSAM-"+"".join(uuid)
  os.system('clear')
  banner()
  info()
...
def menu(my_name,my_id):
	...
	if Meledak in ['1']:
		...
		linex()
		animation(' [×] Successfully Logout+Delete Cookies ')
		exit()
	else:
		linex()
...
def passu():
	...
	linex()
	prints(f' >> On/Off Aeroplane mode if no results ✈️ ')
	linex()
	prog = Progress(TextColumn(' [AHSAM]{task.description}'))
